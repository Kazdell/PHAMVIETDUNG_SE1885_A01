...
{
  public class TagService(ITagRepository repository, IGenericRepository<NewsTag> newsTagRepo) : ITagService
  {
    private readonly ITagRepository _repository = repository;
    private readonly IGenericRepository<NewsTag> _newsTagRepo = newsTagRepo;
    ...
    {
      if (_repository.GetAll().Any(t => t.TagName != null && tag.TagName != null && t.TagName.ToLower() == tag.TagName.ToLower()))
      {
